...
class ApeicPredictor(Predictor):
				...
				for i in xrange(len(indices) - 1):
					for j in xrange(indices[i] + 1, indices[i+1]):
						...
						if successor not in successors:
							app = self.ic.setdefault(successor, App(successor))
							app.pred_co_ocrs[predecessor] += 1.0*math.log(1+j-i)
	...
	def predict(self, session, last, ei, r, k=4):
		ranking = r
		if len(session) == 0:
			results = {}
			for a in ei.keys():
	...
	for user in users:
		...
		for session in testing_sessions:
			...
			for i in xrange(len(session)):
				...
				candidates, ranking = predictor.predict(session[:i], last_session[-1]['application'], ei, ranking, 4)
				if session[i]['application'] in candidates:
					hits += 1.0
				else:
					# if i == 0:
					# 	# print '\n'.join(map(lambda x: x['application'], session[:i]))
					# 	print candidates
